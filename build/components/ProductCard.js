...
const ProductCard = ({ product, cartItems, addToCart, updateCartQuantity, ageConfirmed, onConfirmAge }) => {
    ...
    const getRestsMessage = () => {
        ...
        if (unit === 'Kilogram') {
            if (amount > 5)
                return 'Товара много';
            if (amount > 0)
                return `Осталось ${parseFloat(amount).toFixed(1)} кг`;
            return 'Нет в наличии';
        }
        if (amount > 5)
            return 'Осталось много';
        if (amount > 0)
            return `Осталось ${Math.floor(amount)} шт.`;
        return 'Нет в наличии';
    };
    ...
    return (<div className="product-card">
      <div className="product-card__image-wrapper">
        <img src={imageSrc} loading="lazy" alt={name || 'Товар без названия'} className={`product-card__image ${isAgeRestricted && !ageConfirmed ? 'product-card__image--blur' : ''}`} onError={onImageError}/>
        {isAgeRestricted && !ageConfirmed && (<div className="product-card__age-overlay">
      ...
      <div className="product-card__content">
        ...
        {portion && (<p className="product-card__portion">
        ...
        {isAgeRestricted && !ageConfirmed ? (<button className="product-card__button" onClick={onConfirmAge}>
            Подтвердить возраст
          </button>) : (<>
            {quantity > 0 ? (<div className="quantity-control">
                <button className="btn-quantity" aria-label="Уменьшить количество" onClick={() => updateCartQuantity(product.id, quantity - decrement)} disabled={quantity === 0}>
                  −
                </button>
                ...
                <div className="tooltip">
                  <button className="btn-quantity" aria-label="Увеличить количество" onClick={() => addToCart(product)} disabled={!nextStepAvailable || disableBuy}>
                    +
                  </button>
                  {(!nextStepAvailable && !disableBuy) && (<span className="tooltip-text">Товара нет</span>)}
                </div>
              </div>) : (<button className="product-card__button" disabled={rests === 0 || disableBuy} onClick={() => addToCart(product)} title={disableBuy ? 'Покупка данного товара запрещена' : ''}>
