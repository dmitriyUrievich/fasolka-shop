...
const CartBasketItem = ({ item, updateCartQuantity, removeFromCart }) => {
    ...
    return (<div className="cart-item">
      <img src={imageSrc} alt={item.name || 'Товар'} onError={onImageError} className="cart-item-image" loading="lazy" width="80" height="80"/>
      <div className="cart-item-details">
        <div className="cart-item-header">
        <h4 className="cart-item-name">{item.name}</h4>
          <button className="cart-item-remove-inline" onClick={() => removeFromCart(item.id)} aria-label={`Удалить ${item.name}`}>
              &#10006;
           </button>
          </div>
        ...
        <div className="cart-item-quantity-control">
          <button onClick={() => updateCartQuantity(item.id, item.quantityInCart - step)} disabled={item.quantityInCart <= step} aria-label="Уменьшить количество">
            −
          </button>
          <span>
            {item.unit === 'Kilogram'
           ...
           <div className="tooltip">
              <button onClick={() => updateCartQuantity(item.id, item.quantityInCart + step)} disabled={item.quantityInCart + step > item.rests} aria-label="Увеличить количество">
                +
              </button>
              {/* Показываем подсказку, только если достигнут лимит */}
              {item.quantityInCart + step > item.rests && (<span className="tooltip-text">Товара больше нет</span>)}
          </div>
        </div>
      </div>
    </div>);
