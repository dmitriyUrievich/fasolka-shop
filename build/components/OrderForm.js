...
const OrderForm = ({ onSubmit, onClose }) => {
    ...
    const validate = useCallback(() => {
        ...
        if (!deliveryTime)
            newErrors.deliveryTime = 'Выберите интервал доставки.';
        setErrors(newErrors);
    ...
    return (<div className="order-form-container">
      ...
      <h2 id="modal-title">Оформление заказа</h2>
        ...
        <div className="form-actions"> 
          ...
          <button type="button" className="payment-button main-card" disabled={isSubmitting} onClick={() => handleProcessPayment('Card')}>
            {isSubmitting ? 'Отправка...' : 'Оплатить картой'}
          </button>
          <button type="button" className="payment-button sberpay" onClick={() => handleProcessPayment('SberPay')}>
            Оплатить через SberPay
          </button>
          <button type="button" className="payment-button psb" onClick={() => handleProcessPayment('PSB')}>
            Оплатить через ПСБ
          </button>
        </div>
      </div>
