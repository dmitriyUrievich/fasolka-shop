...
const ProductList = ({ products, loading, searchTerm, sortOption, cartItems, updateCartQuantity, addToCart, selectedCategoryId, listHeader, }) => {
    ...
    const filteredProducts = useMemo(() => {
        return products.filter((product) => {
            if (!product)
                return false;
            if (product.productType === 'Tobacco')
                return false;
    ...
    if (sortedProducts.length === 0) {
        return (<div className="no-products-message">
    ...
    return (<>
      <div className="product-list-container">
        ...
        <div className="product-grid">
          {currentItems.map((product) => (<ProductCard key={product.id} product={product} addToCart={addToCart} ageConfirmed={ageConfirmed} onConfirmAge={handleConfirmAge} cartItems={cartItems} updateCartQuantity={updateCartQuantity}/>))}
        </div>

        <div className="pagination-footer">
          <Pagination itemsPerPage={itemsPerPage} totalItems={sortedProducts.length} paginate={paginate} currentPage={currentPage}/>
        </div>
      </div>
     <div className="map-section">
        <h2 className="map-title">Наш Магазин на Карте</h2>
        <Map center={[44.675898, 37.642492]} zoom={12} placemark={[44.675898, 37.642492]} placemarkHint="Фасоль" placemarkBalloon="Мы находимся по адресу: пер. Торпедный д4."/>
      </div>
    </>);
};
