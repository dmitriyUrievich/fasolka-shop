...
const CartBasket = ({ cartItems = [], removeFromCart, onClearCart, updateCartQuantity, onClose, onProceedToOrder }) => {
    // 🔹 Мемоизация суммы
    const subtotal = useMemo(() => {
        return cartItems.reduce((sum, item) => sum + (item.sellPricePerUnit || 0) * item.quantityInCart, 0);
    }, [cartItems]);
    // 🔹 Мемоизация доставки
    ...
    return (<div className="cart-container">
      ...
      <button className="cart-clear-button" onClick={onClearCart} disabled={cartItems.length === 0} title={cartItems.length === 0 ? 'Корзина уже пуста' : 'Очистить корзину'}>
        Очистить
      </button>

      {cartItems.length === 0 ? (<p className="cart-empty-message">Ваша корзина пуста.</p>) : (<div className="cart-items-list">
          {cartItems.map((item) => (<CartBasketItem key={item.id} item={item} removeFromCart={removeFromCart} updateCartQuantity={updateCartQuantity}/>))}
        </div>)}
      ...
      <div className="cart-summary">
        ...
        {showFreeHint && (<p className="cart-delivery-hint">
            💡 Добавьте ещё на{' '}
            <strong>{(3000 - subtotal).toLocaleString('ru-RU')} ₽</strong>,{' '}
            чтобы получить <strong>бесплатную доставку</strong>!
          </p>)}

        {isOrderValid && (<p className="cart-total">
            Итого: <span>{total.toLocaleString('ru-RU')} ₽</span>
          </p>)}

        <button className="cart-checkout-button" disabled={!isOrderValid} onClick={isOrderValid ? onProceedToOrder : undefined}>
...
export default React.memo(CartBasket, (prevProps, nextProps) => {
    return (prevProps.cartItems.length === nextProps.cartItems.length &&
        prevProps.cartItems.every((item, i) => item.id === nextProps.cartItems[i]?.id &&
