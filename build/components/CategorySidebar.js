...
const CategorySidebar = ({ categories, products, activeCategoryId, onCategorySelect }) => {
    ...
    const isBlacklisted = (name) => ['ОБОРУДОВАНИЕ', 'Вода 19л', 'ПИКНИК', 'ХОЛОДНЫЙ ЧАЙ', 'Без группы', 'Пасха', 'Сигареты']
    ...
    return (<div className="category-sidebar">
      ...
      <div className="category-sidebar__scrollable">
        {topLevel.map(parent => (<React.Fragment key={parent.id}>
            <div className={`category-item ${isActive(parent.id) ? 'active' : ''}`} onClick={() => handleParentClick(parent.id)}>
              <div className="category-item-content">
            ...
            {parent.children.length > 0 && expandedCategories.has(parent.id) && (<ul className="sub-category-list">
                {parent.children.map(child => (<li key={child.id} className={`sub-category-item ${isActive(child.id) ? 'active' : ''}`} onClick={(e) => {
                        e.stopPropagation();
